...
NUM_CARS = 3  # Supports key control of two cars, but can simulate as many as needed
NUM_ACTIONS = 5
...
def key_press(k, mod):
    ...
    if k==CAR_CONTROL_KEYS[0]:  a[-1][0] = -1.0
    if k==CAR_CONTROL_KEYS[1]:  a[-1][0] = +1.0
    if k==CAR_CONTROL_KEYS[2]:  a[-1][1] = +1.0
    if k==CAR_CONTROL_KEYS[3]:  a[-1][2] = +0.8

def key_release(k, mod):
    global CAR_CONTROL_KEYS
    # Iterate through cars and assign them control keys (mod num controllers)
    if k==CAR_CONTROL_KEYS[0]  and  a[-1][0]==-1.0:  a[-1][0] = 0
    if k==CAR_CONTROL_KEYS[1] and  a[-1][0]==+1.0:  a[-1][0] = 0
    if k==CAR_CONTROL_KEYS[2]:     a[-1][1] = 0
    if k==CAR_CONTROL_KEYS[3]:   a[-1][2] = 0
